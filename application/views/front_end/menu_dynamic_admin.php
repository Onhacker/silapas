...
<?php if ($this->session->userdata("admin_login") == true): ?>
    ...
    <li class="has-submenu <?= ($uri == 'admin_scan/') ? 'active-menu' : '' ?>">
        <a href="<?= site_url("admin_scan/") ?>"><i class="fe-book-open"></i> Checkin/Checkout</a>
    </li>
    <li class="has-submenu <?= ($uri == 'admin_dashboard/monitor') ? 'active-menu' : '' ?>">
        <a href="<?= site_url("admin_dashboard/monitor") ?>"><i class="fe-eye"></i> Monitoring</a>
    </li>
