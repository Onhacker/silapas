<?php if ($this->uri->segment(1) != 'on_login'): ?>
  ...
  <style>
    ...
    @media (max-width: 576px){ #playstoreBadge img { height: 48px; } }
  </style>

  <style type="text/css">
    .share-buttons{ display:flex; align-items:center; justify-content:center; flex-wrap:wrap; gap:10px; margin-top:10px; }
    .share-buttons button{ display:flex; align-items:center; gap:6px; border:none; border-radius:6px; color:#fff; font-weight:500; cursor:pointer; transition:.3s; }
    .btn-whatsapp{ background-color:#25D366; } .btn-whatsapp:hover{ background-color:#1ebe5b; }
    .btn-facebook{ background-color:#3b5998; } .btn-facebook:hover{ background-color:#334b86; }
    .btn-twitter{ background-color:#1DA1F2; } .btn-twitter:hover{ background-color:#198dd6; }
    .btn-telegram{ background-color:#0088cc; } .btn-telegram:hover{ background-color:#007ab8; }
      ...
      <div class="col-12">
        <div class="card-box-carbul">
          ...
          <div class="text-center">
            ...
            <style>
              :root{ --badge-h:35px; }
              #playstoreBadge img,#installButton img{ height:var(--badge-h)!important; width:auto!important; max-height:none!important; display:inline-block; vertical-align:middle; }
              .store-badges a + a{ margin-left:.5rem; }
            </style>
            <div class="text-center store-badges">
              <a id="playstoreBadge" href="<?= $playUrl ?>" onclick="return openPlayStore(event)" class="d-inline-block my-2" aria-label="Download di Google Play">
                <img alt="Download di Google Play" src="<?= base_url('assets/images/gp.webp') ?>" style="height:56px;width:auto;">
              </a>
              <a id="installButton" href="#" class="d-inline-block my-2 ms-2 ml-2" aria-label="Instal ke iOS (PWA)">
                <img alt="Instal ke iOS (Tambahkan ke Layar Utama)" src="<?= base_url('assets/images/ios.webp') ?>" style="height:56px;width:auto;">
              </a>
            </div>
          </div>

          <style type="text/css">
            .divider{ border:0; height:1px; background-color:rgba(0,0,0,.05); margin:20px 0; border-radius:1px; }
...
<style>
  /* === CSS ASLI NAVBAR BAWAH (tidak diubah) === */
  .navbar-bottom{ height:65px; border-top:1px solid #dee2e6; background-color:#fff; box-shadow:0 -1px 5px rgba(0,0,0,0.05); z-index:1030; }
  .navbar-bottom a{ font-size:12px; color:#666; text-decoration:none; }
  .navbar-bottom i{ font-size:18px; }
  .center-button{ position:absolute; top:-25px; left:50%; transform:translateX(-50%); background-image:linear-gradient(to right,#1e3c72 0%,#2a5298 100%)!important; width:65px; height:65px; border-radius:50%; border:1px solid #dee2e6; box-shadow:0 4px 10px rgba(0,0,0,0.2); display:flex; align-items:center; justify-content:center; animation:footerAni 1s infinite; z-index:10; }
  ...
  .modal-dialog.modal-bottom{ position:fixed; bottom:0; left:0; right:0; transform:translateY(100%); transition:transform .3s ease-out; margin:0; }
  ...
  .navbar-bottom .nav-item a:hover i,.navbar-bottom .nav-item a:hover span{ color:#4a81d4!important; }
...
<nav class="navbar fixed-bottom navbar-light bg-white shadow-sm d-lg-none navbar-bottom px-0">
  <div class="w-100 d-flex justify-content-between text-center position-relative mx-0 px-0">
    ...
    <div>
      <a href="<?= base_url('booking') ?>" class="center-button <?= ($uri=='booking') ? 'text-white' : '' ?>" style="text-align:center; <?= ($uri=='booking') ? 'background-color:#28a745;' : '' ?>">
        <img src="<?= base_url('assets/images/logo.png') ?>" alt="Permohonan" style="width:50px; height:50px; object-fit:contain; margin-top:0;">
      </a>
    </div>
...
<style type="text/css">
  ...
  .modal-content{ border-radius:0; width:100%; margin:0; }
  ...
  <div class="modal-dialog modal-dialog-scrollable modal-bottom fadeInUp animated modal-dialog-full" style="animation-duration:.5s;">
    <div class="modal-content">
      <div class="modal-header bg-blue text-white">
        <h5 class="modal-title d-flex align-items-center text-white" id="menumoLabel"><i class="fas fa-concierge-bell mr-2"></i> Menu</h5>
...
<!-- Style khusus (ASLI, tidak diubah) -->
<style>
  #kontakModalfront .menu-list{ max-height:70vh; overflow-y:auto; padding:12px; }
  #kontakModalfront .menu-item{ display:flex; align-items:center; gap:10px; padding:12px 14px; margin:10px 12px; border-radius:12px; background:#c7d5ff; font-weight:600; color:#111 !important; text-decoration:none !important; transition:background .2s ease, transform .1s ease; }
