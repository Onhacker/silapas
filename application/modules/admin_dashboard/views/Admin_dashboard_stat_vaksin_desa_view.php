...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        <?php } ?>
        $('.spinner-grow').hide(); 
        $('#id_dusun,#jenis_vaksin,#tahun,#bulan_awal').select2();

    });
    function reset(){
         $('#jenis_vaksin,#tahun,#bulan_awal,#id_dusun').val('x').trigger('change');
         <?php if ($this->session->userdata("admin_level") != "admin") {?>
            load_stat_vaksin();      
        <?php } ?>
    }
    function load_stat_vaksin() {
        ...
        <?php if ($this->session->userdata("admin_level") == "admin") {?> 
            ...
            if (tahun =="" || id_dusun == "x" ) {
                ...
                $.ajax({
                    <?php if ($this->session->userdata("admin_level") == "admin") {?> 
                       url :'<?php echo site_url(strtolower($controller)."/load_stat_vaksin_desa"); ?>/'+jenis_vaksin+'/'+tahun+'/'+bulan_awal+'/'+id_dusun,
                   <?php } else {?>
