...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        load_stat_vaksin();
        $('.spinner-grow').hide(); 
        $('#id_dusun,#id_desa_cari,#tahun,#jenis_vaksin').select2();

    });
    function reset(){
       $('#id_desa_cari,#tahun,#jenis_vaksin,#id_dusun').val('x').trigger('change');
       load_stat_vaksin();
    }
    function load_stat_vaksin() {
        ...
        <?php } ?>
            if (tahun =="" ) {
                ...
                $.ajax({
                    <?php if ($this->session->userdata("admin_level") == "admin") {?> 
                       url :'<?php echo site_url(strtolower($controller)."/load_trend_vaksin"); ?>/'+id_desa_cari+'/'+tahun+'/'+jenis_vaksin+'/'+id_dusun,
                   <?php } else {?>
