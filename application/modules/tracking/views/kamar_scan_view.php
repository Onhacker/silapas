...
/* Ribbon nomor urut di kanan atas setiap kartu tahanan */
/* Ribbon nomor urut di kanan atas setiap kartu tahanan, model pill seperti contoh */
/* Ribbon nomor urut di kanan atas setiap kartu tahanan, non-pill */
/* Ribbon nomor urut di kanan atas setiap kartu tahanan, model "terpisah" */
.tahanan-card{
  position:relative;
  overflow:visible; /* biar bagian ribbon yang keluar card tetap kelihatan */
}

.tahanan-ribbon{
  position:absolute;
  top:4px;
  right:-8px;       /* minus = sedikit keluar ke kanan */
  z-index:3;
  pointer-events:none;
...
.tahanan-ribbon-box{
  ...
  text-align:center;
  box-shadow:none;  
  border-radius: 3px 0 0 3px;         /* buang shadow */
}

/* segitiga kecil di ujung kanan bawah */
.tahanan-ribbon-box::after{
  content:"";
  position:absolute;
...
<div class="container-fluid">
          ...
          <div class="tahanan-list">
            <?php $no = 1; foreach($tahanan as $t): ?>
              <?php
                ...
                $fotoFile = trim((string)($t->foto ?? ''));

                $fotoThumb = null;
                $fotoFull  = null;

                if ($fotoFile !== '') {
                    $pathFull  = FCPATH.'uploads/kamar_tahanan/'.$fotoFile;
                    $pathThumb = FCPATH.'uploads/kamar_tahanan/thumb/'.$fotoFile;

                    if (is_file($pathFull)) {
                        $fotoFull = base_url('uploads/kamar_tahanan/'.rawurlencode($fotoFile));
                    }

                    // kalau thumbnail ada → pakai thumb
                    if (is_file($pathThumb)) {
                        $fotoThumb = base_url('uploads/kamar_tahanan/thumb/'.rawurlencode($fotoFile));
                    }
                    // kalau thumbnail belum ada tapi fotonya ada → pakai foto full dulu
                    elseif ($fotoFull) {
                        $fotoThumb = $fotoFull;
                    }
                }
  ...
  <!-- Ribbon nomor urut, kotak kanan atas -->
  <div class="tahanan-ribbon">
    <div class="tahanan-ribbon-box">
      <?= $no; ?>
    </div>
  </div>
...
<div class="modal fade" id="photoModal" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content bg-dark text-white">
      ...
      <div class="modal-body modal-photo-body text-center">
        <!-- SKELETON SHIMMER -->
        <div id="photoSkeleton" class="photo-skeleton"></div>

        <!-- FOTO ASLI -->
        <img src="" alt="Foto WBP" id="photoModalImg" class="modal-photo-img d-none">
...
<script>
// Klik foto -> buka modal, tampilkan skeleton, lalu load foto full
document.addEventListener('click', function(e){
  ...
  var skeleton = document.getElementById('photoSkeleton');

  if (!modalImg || !skeleton) return;
  ...
  // Preload image "full" secara async
  ...
  loader.onerror = function(){
    // kalau gagal, tetap sembunyikan skeleton dan coba tampilkan apa adanya
    skeleton.style.display = 'none';
    modalImg.src = full;
    modalImg.classList.remove('d-none');
  };
